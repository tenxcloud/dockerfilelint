...
module.exports.run = function(configPath, content, configContent) {
  ...
  var state = {
    ...
    items: [],
    rules: loadRules(configPath, configContent)
  }
...
function loadRules(configPath, configContent) {
  if (configContent) {
    var rc = yaml.safeLoad(configContent)
    return rc.rules
  }
    ...
    switch (cmd) {
      ...
      case 'expose':
        ...
        args.match(/\S+/g).forEach(function(port) {
          if (!checks.expose_port_valid(port) && !checks.expose_port_valid_is_variable(port)) {
